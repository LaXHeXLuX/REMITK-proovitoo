...
@Component({
  ...
  template: `
    <div class="unit-form">
      <h3>{{ isEdit ? 'Edit Unit' : 'Create Unit' }}</h3>
      ...
      <div>
        <label>Licence
          <input [(ngModel)]="formLicence" />
        </label>
      </div>
      <div>
        <label>VIN
          <input [(ngModel)]="formVin" />
        </label>
      </div>
      <div>
        <label>Price per day
          <input type="number" [(ngModel)]="formPrice" />
        </label>
      </div>
      <div>
        <label>Bookable
          <input type="checkbox" [(ngModel)]="formBookable" />
        </label>
      </div>
      <div class="actions">
  ...
  `,
  styles: [`.unit-form { margin-bottom:12px } .actions button { margin-right:8px }`]
})
